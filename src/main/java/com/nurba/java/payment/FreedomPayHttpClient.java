...
public class FreedomPayHttpClient {
    ...
    public FreedomPayInitResult initPayment(Long orderId, BigDecimal amount, String description) {
        if (props.getMerchantId().isBlank()) {
            return stub(orderId);
        }
        ...
        try {
            ...
            log.debug("[FreedomPay] init response: {}", xml);
    ...
    private FreedomPayInitResult stub(Long orderId) {
        log.info("[FreedomPay] Stub mode (merchantId not configured). orderId={}", orderId);
        String base = props.getSuccessUrl().isBlank()
                ? "https://pay.freedompay.kz/stub"
                : props.getSuccessUrl();
        String sep = base.contains("?") ? "&" : "?";
        String redirectUrl = base + sep + "orderId=" + orderId;
        return new FreedomPayInitResult("stub-fp-" + orderId, redirectUrl, true, null);
    }
    ...
    static FreedomPayInitResult parseAndVerify(String xml, String secretKey) {
        try {
            ...
            if (!FreedomPaySignature.verify("init_payment.php", fields, secretKey, receivedSig)) {
                log.warn("[FreedomPay] pg_sig mismatch in init_payment.php response " +
            ...
            String error = fields.get("pg_description");
            if (error == null) error = fields.get("pg_error_description");
            log.warn("[FreedomPay] init returned pg_status={} description={}", status, error);
            return new FreedomPayInitResult(null, null, false, error);
