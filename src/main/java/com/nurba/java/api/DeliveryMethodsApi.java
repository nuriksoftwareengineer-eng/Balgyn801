...
public interface DeliveryMethodsApi {
    ...
    @Operation(
        ...
        description = "страна → тарифная зона → официальный тариф Казпочты по весу (всегда AIR — "
                    + "единственный способ международной доставки). Вес считается на бэкенде из "
                    + "переданных позиций корзины — клиент вес не передаёт. POST, так как тело "
                    + "содержит список позиций."
    )
