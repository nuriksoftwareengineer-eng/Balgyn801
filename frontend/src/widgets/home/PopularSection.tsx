...
import { getPopularDesigns } from "@/shared/api/backend-api";
...
export function PopularSection() {
  ...
  const { data } = useQuery({ queryKey: ["popular-designs"], queryFn: () => getPopularDesigns(8), staleTime: 5 * 60_000 });

  if (!data || data.length === 0) return null;

  return (
    <section className="py-14 md:py-20 bg-[--color-surface]">
      <div className="mx-auto max-w-[1440px] px-4 md:px-6">
        <div className="mb-8 flex items-center justify-between">
          <h2 className="text-xl font-semibold uppercase tracking-[0.12em]">{t("home.popular", "Популярное")}</h2>
