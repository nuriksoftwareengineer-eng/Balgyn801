...
/** Главный экран: видео-фон + затемнение + логотип-вордмарк + CTA. */
  ...
  return (
    <section className="relative min-h-[88vh] w-full bg-black text-white overflow-hidden flex flex-col justify-center items-center">
      <div className="absolute inset-0 h-full w-full overflow-hidden pointer-events-none">
        {/* Video background; poster = fallback image if video fails (Firefox/no-codec) */}
        <video
          autoPlay
          muted
          loop
        ...
        >
          {/* H.264 inside QuickTime container — Chrome/Edge accept video/mp4 hint */}
          <source src={heroVideo} type="video/mp4" />
          <source src={heroVideo} type="video/quicktime" />
          {/* Hard fallback for browsers that don't support video at all */}
          <img src={heroImg} alt="BALGYN — кампания SS26" className="h-full w-full object-cover" />
        </video>
      </div>
      ...
      <div className="relative z-10 flex w-full flex-col items-center justify-center px-4 md:px-8">
        ...
        >
          Приносим свежесть в твой стиль.
        </motion.p>
