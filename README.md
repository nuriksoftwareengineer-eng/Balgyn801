...
> Файл `.env` не нужен. Все платёжные системы работают в stub-режиме (заказы подтверждаются автоматически). Для production-настройки смотрите раздел [Environment Variables](#environment-variables).
...
Все переменные имеют dev-дефолты — `.env` файл не требуется для локальной разработки.  
В production обязательно замените `JWT_SECRET`, `BOOTSTRAP_ADMIN_*` и `POSTGRES_PASSWORD`.

### Security

| Переменная | Дефолт (dev) | Описание |
|-----------|-------------|---------|
| `JWT_SECRET` | `balgyn-dev-jwt-secret-...` | Секрет JWT (HS256). **ОБЯЗАТЕЛЬНО замените в prod.** Генерация: `openssl rand -hex 32` |
...
Stub-режим активен когда `FREEDOMPAY_MERCHANT_ID` пуст. Заказы подтверждаются автоматически локально.

| Переменная | Дефолт | Описание |
|-----------|--------|---------|
| `FREEDOMPAY_MERCHANT_ID` | пусто → **stub** | ID мерчанта Freedom Pay |
| `FREEDOMPAY_SECRET_KEY` | пусто → **stub** | Секретный ключ |
| `FREEDOMPAY_RESULT_URL` | пусто | URL callback (нужен только для prod, публично доступный) |
| `FREEDOMPAY_TESTING_MODE` | `false` | `true` для sandbox Freedom Pay |

### PayPal (оплата — international)

Stub-режим активен когда `PAYPAL_CLIENT_ID` пуст. Заказы подтверждаются автоматически локально.

| Переменная | Дефолт | Описание |
|-----------|--------|---------|
| `PAYPAL_MODE` | `sandbox` | `sandbox` или `live` |
| `PAYPAL_CLIENT_ID` | пусто → **stub** | Client ID из PayPal Developer |
| `PAYPAL_CLIENT_SECRET` | пусто → **stub** | Client Secret |
| `PAYPAL_WEBHOOK_ID` | пусто | Webhook ID для верификации webhook-событий |
